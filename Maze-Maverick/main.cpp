...
void c_col(int c){
	...
	if(c == 1)
    		...
    		cout << (ch? "\033[47m\033[47m": "\033[0m\033[0m");
    #endif
    return;
}
...
void leaderboard(){
	...
	for(int i = 0; true; ++i){
		...
		l = max(1LL, (int)ceil(log10(abs(standing[i].second) + 1)) + (int)(standing[i].second < 0));
		for(int j = 0; j < 6 - l; ++j)
			cout << " ";
		c_col(15);
		cout << "|" << '\n';
...
void menu(){
	...
	for(bool b = false; true; b = true){
		...
		if(s[0] == '8')
			c_col(15);
		cout << "----------\nIf you want to select an option write it's section number\nAfter you set the section you want to go press enter key\n----------" << '\n';
